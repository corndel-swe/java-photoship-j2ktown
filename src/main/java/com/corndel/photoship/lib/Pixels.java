...
public class Pixels {
    ...
    public static List<Integer> blackAndWhite(List<Integer> rgb) {
        ...
        if (average < 128) {
            rgb.replaceAll(num -> num = 0);
            return rgb;
        }
        rgb.replaceAll(num -> num = 255);
        return rgb;
    ...
    public static List<Integer> sepia(List<Integer> rgb) {
        ...
        rgb.set(0, Math.min(255, Math.max(0, (int) red)));
        rgb.set(1, Math.min(255, Math.max(0, (int) green)));
        rgb.set(2, Math.min(255, Math.max(0, (int) blue)));
    ...
    public static List<Integer> adjustBrightness(List<Integer> rgb, int brightness) {
        ...
        for (int i = 0; i < rgb.size(); i++) {
            int newNum = rgb.get(i) + brightness;
            rgb.set(i, Math.min(255, Math.max(0, newNum)));
        }
        return rgb;
    }
}
